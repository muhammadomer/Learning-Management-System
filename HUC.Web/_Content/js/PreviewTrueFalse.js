function PreviewTrueFalse(jasonObj) {
...
<p id="demo"></p>`;
        ...
        $("#" + this.jasonObj.divId).on("drop", (event) => {
            ...
            if ($(event.target).hasClass("droptarget")) {
                ...
                if (questionLength == 0) {
                    ...
                    btnSubmit.prop("disabled", true); // Naqi Changes for Preview
                }
                $("#" + this.jasonObj.divId + "-true")
                    .parent()
                    .parent()
    ...
    let correctIcon = (e) => {
        $(e.target).siblings("div.ev-icon-check").css("display", "none");
    };
}

//This for Multiple Choices

function PreviewTrueFalse2(jasonObj) {
    this.jasonObj = jasonObj;
    var autoTrigger = 0;
    let remainingAttempts = this.jasonObj.attempts;
    let title = this.jasonObj.title;
...
function PreviewTrueFalse1(jasonObj) {
    debugger
    this.jasonObj = jasonObj;
    let remainingAttempts = this.jasonObj.attempts;
    let title = this.jasonObj.title;
            ...
            if ($.trim($(e.target).children().text()) === "Submit") {
                ...
                if (s == length) {
                    ...
                    if (remainingAttempts <= 0) {
                        ...
                        // main page change text remaining attempts to final attempts
                        if (s > 0) {
                           // $("#ev-prompt-body-" + this.jasonObj.divId).text(popUpMessage4);
                        } else {
