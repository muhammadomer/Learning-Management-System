...
{
    public class ChunkedUploadEditController : ApiController
...

    {
        ...
        public HttpResponseMessage CommitChunks([FromUri] string dzIdentifier, [FromUri] string fileName,
            [FromUri] int expectedBytes, [FromUri] int totalChunks, [FromUri] string ChapterId, [FromUri] int Idd)
        {
            ...
            //ChapterContentEditModel model = new ChapterContentEditModel();
           ...
           // model.ID = Id;
            ...
            Database.Execute("UPDATE ChapterContents SET value=@Value where id=@ID", new { ID = Idd,value=fileName });
