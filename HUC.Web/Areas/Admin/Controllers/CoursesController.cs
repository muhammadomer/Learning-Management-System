...
namespace HUC.Web.Areas.Admin.Controllers
{
    public class CoursesController : AdminBaseController
    {
        ...
        public ActionResult Create(CourseAddModel model)
        {
            ...
            if (ModelState.IsValid)
            {
              ...
              int lastrecord = Database.ExecuteInsert(model,true);
               // int lastrecord = Database.Query<int>("select Max(ID) from Courses where IsCreatedBy = 0").First();
                ...
                resourcemodel.CourseID = model.ID;
                        ...
                        if(item.Questions != null && item.Questions.Count() > 0)
                        {
                            foreach (var ques in item.Questions)
                            {
                                ...
                                if (ques.Answers != null && ques.Answers.Count() > 0)
                                        {
        ...
        //public bool SaveAsNewFile(string fileName , string errorMessage , HttpPostedFileBase file)
        //{
        //    try
        //    {
